...
void FT::InverseFastFourierTransform(int ** InputImage, int ** OutputImage, double ** FreqReal, double ** FreqImag, int h, int w)
{
	...
	{
		std::complex<double> *input = new std::complex<double>[M];
		for (int j = 0; j < M; j++) input[j] = std::complex<double>(outputComplex[i][j].real()*N, outputComplex[i][j].imag()*N);
		InverseFFT(M, input);
		for (int j = 0; j < M; j++) outputComplex[i][j] = input[j];
		free(input);
	}
	for (int i = 0; i < N; i++) 
	{
		std::complex<double> *input = new std::complex<double>[N];
		for (int j = 0; j < N; ++j) input[j] = std::complex<double>(outputComplex[j][i].real()*N, outputComplex[j][i].imag()*N);
		InverseFFT(N, input);
		for (int j = 0; j < N; ++j) outputComplex[j][i] = input[j];
		free(input);
	}
	double max_s = -100;
	for (int i = 0; i < M; i++) 
